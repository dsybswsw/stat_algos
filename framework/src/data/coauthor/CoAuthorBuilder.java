...
public class CoAuthorBuilder {
	...
	public void addFriendshipRelations(String friendshipDataFile) throws IOException {
		...
		while (lineReader.hasNext()) {
			...
			DataArc arc = new DataArc(startNode, endNode, new DataArcContent(CoAuthorConstants.FRIENDSHIP, arcValue));
	...
	public void addAuthorRelations(String authorArticleFile) throws IOException {
		...
		while (lineReader.hasNext()) {
			...
			DataArc arc = new DataArc(startNode, endNode, new DataArcContent(CoAuthorConstants.AUTHOR_ARTICAL, arcValue));
			startNode.addNeighbour(arc);
			++index;
			if (index % LOG_MARGGIN == 0) {
	...
	public void addRatingRelations(String ratingFile) throws IOException {
		...
		while (lineReader.hasNext()) {
			...
			DataArc arc = new DataArc(startNode, endNode, new DataArcContent(CoAuthorConstants.REVIEM, arcValue));
			startNode.addNeighbour(arc);
			++index;
			if (index % LOG_MARGGIN == 0) {

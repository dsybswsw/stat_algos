 ...
 */
public class MaxEntJob extends Configured implements Tool {
    public static int param_num = 4;
    public static int class_num = 2;
    public static int max_iter = 20;

    public static void main(String[] args) {
        try {
            ToolRunner.run(new MaxEntJob(), args);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private int runOneIter(int iter, String in, String out) throws Exception {
        Job job = new Job(getConf(), "maxent");
        job.setJarByClass(MaxEntJob.class);
        Configuration conf = job.getConfiguration();
        conf.setInt(SGDUtils.PARAM_NUM, param_num);
        conf.setInt(SGDUtils.CLASS_NUM, class_num);
        if (iter == 0) {
        ...
        job.setMapperClass(MaxEntMapper.class);
    ...
    public int run(String[] args) throws Exception {
        if (args.length != 4) {
            System.out.println("./exe feat_num class_num in out");
            return -1;
        }
        MaxEntJob.param_num = Integer.parseInt(args[0]);
        MaxEntJob.class_num = Integer.parseInt(args[1]);
        MaxEntJob.param_num = MaxEntJob.param_num * MaxEntJob.class_num;
